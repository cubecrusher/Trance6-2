...
public class GameScreen extends ScreenAdapter {
    ...
    private TextureRegion backr;
    private TextureRegionDrawable backrd;
    ...
    public String highScoreg;
    public String strDouble;
    ...
    public void show(){
        ...
        if (settings.isMusicOn()) {
            ...
            else if (rng<=0.4 && rng>0.2) {
                Assets.chaozFantasy.play();
                bpm = 108;
                musicDur = 121;
                musicN = 1;
            }
            else if (rng<=0.6 && rng>0.4) {
                Assets.fireFly.play();
                bpm = 140;
                musicDur = 258;
                musicN = 2;
            }
            else if (rng<=0.8 && rng>0.6) {
                Assets.uD.play();
                bpm = 140;
                musicDur = 198;
                musicN = 3;
    ...
    public void render(float delta){
        ...
        update();
        this.playTime += delta;
            this.strDouble = String.format("%.2f", playTime);
            if (Float.parseFloat(this.strDouble) > Float.parseFloat(settings.getHighScore())) highScore = playTime;
            musicPos+=Float.parseFloat(this.strDouble);
            ...
            EndScreen.best = Float.parseFloat(highScoreg);
            ...
            if (playTime<Float.parseFloat(settings.getHighScore())) {
            ...
            if (player.nn>6000) {
                ...
                EndScreen.time = (Float.parseFloat(this.strDouble));
                if (playTime > Float.parseFloat(this.highScoreg)) {
                    settings.setHighScore(String.format("%.2f", playTime));
