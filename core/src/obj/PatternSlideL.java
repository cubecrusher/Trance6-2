...
public class PatternSlideL {
    ...
    public PatternSlideL(GameScreen gameScreen){
        this.x = 0;
    ...
    public void update(){
        if (y<-200-TrJr.INSTANCE.getScrW()) isOut = true;
            ...
            verticesL[2] = 0;
            verticesL[3] = y + 3 * TrJr.INSTANCE.getScrW();

            verticesL[4] = 4 * TrJr.INSTANCE.getScrW() / 5f;
            verticesL[5] = y + TrJr.INSTANCE.getScrW() / 5f;

            verticesL[6] = 4 * TrJr.INSTANCE.getScrW() / 5f;
            verticesL[7] = y;


            verticesR[0] = TrJr.INSTANCE.getScrW() / 5f;
            verticesR[1] = y + TrJr.INSTANCE.getScrW();

            verticesR[2] = 4 * TrJr.INSTANCE.getScrW() / 5f;
            verticesR[3] = y + TrJr.INSTANCE.getScrW();

            verticesR[4] = TrJr.INSTANCE.getScrW();
            verticesR[5] = y + 2*TrJr.INSTANCE.getScrW()/5f;

            verticesR[6] = TrJr.INSTANCE.getScrW();
            verticesR[7] = y+TrJr.INSTANCE.getScrW();
